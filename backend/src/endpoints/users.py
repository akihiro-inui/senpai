...
from ..schemas.users import UserSchema, UserCreate, UserUpdate, UserGet
...
def get_all_users_endpoint(db_session: Session = Depends(DBC.get_session)):
    ...
    """
    return get_all_users_from_db(db_session)
...
def get_one_user_by_email_endpoint(user_email: str, db_session: Session = Depends(DBC.get_session)):
    ...
    """
    # Get user by name
    return get_user_by_email_from_db(user_email, db_session)
